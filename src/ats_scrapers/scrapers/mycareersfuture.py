...
class MyCareersFutureScraper(BaseScraper):
    ...
    async def afetch(self) -> list[Job]:
        ...
        if max_pages_env:
            try:
                ...
                max_pages = None
        return await self._fetch_async(user_agent, max_pages)
        ...
        async with httpx.AsyncClient(
            ...
            for start in range(0, len(offsets), MAX_CONCURRENCY):
                ...
                results = await asyncio.gather(
                    *(fetch_one(offset) for offset in offset_batch)
                )
    ...
    def _parse_job(self, item: dict[str, Any]) -> Job | None:
        ...
        country_iso = None if is_overseas else "SG"
        region = None if is_overseas else "Asia"
...
def _build_raw(
    ...
    if is_overseas:
        raw["isOverseas"] = True
    # Defensive size cap — remove verbose optional fields until the
    # serialized payload is guaranteed to fit the schema budget.
    if raw:
        import json
        ...
        for key in (
            ...
            if len(json.dumps(raw, default=str)) <= MAX_RAW_SERIALIZED:
                break
            raw.pop(key, None)
        while raw and len(json.dumps(raw, default=str)) > MAX_RAW_SERIALIZED:
            largest = max(
                raw,
                key=lambda key: len(json.dumps(raw[key], default=str)),
            )
            raw.pop(largest)
    return raw
