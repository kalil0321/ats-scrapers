    ...
    async def _fetch_async(self) -> list[Job]:
        ...
        async with httpx.AsyncClient(
            ...
            if self.max_pages is not None:
                page_numbers = page_numbers[: max(0, self.max_pages - 1)]
            ...
            for start in range(0, len(page_numbers), MAX_CONCURRENCY):
                page_batch = page_numbers[start : start + MAX_CONCURRENCY]
                await asyncio.gather(*(fetch_one(page) for page in page_batch))
